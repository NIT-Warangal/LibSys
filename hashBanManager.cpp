...
#ifdef _WIN32
    if(sReason != NULL) {
        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sReason) == 0) {
...
#ifdef _WIN32
    if(sBy != NULL) {
        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sBy) == 0) {
...
#ifdef _WIN32
                            Ban->sNick = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szNickLen+1);
#else
...
#ifdef _WIN32
                        Ban->sReason = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szReasonLen+1);
#else
...
#ifdef _WIN32
                        Ban->sBy = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szByLen+1);
#else
...
#ifdef _WIN32
                        RangeBan->sReason = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szReasonLen+1);
#else
...
#ifdef _WIN32
                        RangeBan->sBy = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szByLen+1);
#else
...
void clsBanManager::Ban(User * u, const char * sReason, char * sBy, const bool &bFull) {
    ...
    if(u->sNick[0] != '<') {
#ifdef _WIN32
        pBan->sNick = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, u->ui8NickLen+1);
#else
...
#ifdef _WIN32
        pBan->sReason = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szReasonLen > 255 ? 256 : szReasonLen+1);
#else
...
#ifdef _WIN32
        pBan->sBy = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szByLen+1);
#else
...
#ifdef _WIN32
        pBan->sNick = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szNickLen+1);
#else
...
void clsBanManager::TempBan(User * u, const char * sReason, char * sBy, const uint32_t &minutes, const time_t &expiretime, const bool &bFull) {
    ...
    if(expiretime > 0) {
        ...
        if(minutes > 0) {
    	    ...
    	    pBan->tempbanexpire = acc_time+(clsSettingManager::mPtr->iShorts[SETSHORT_DEFAULT_TEMP_BAN_TIME]*60);
        }
    }
...
bool clsBanManager::RangeBan(char * sIpFrom, const uint8_t * ui128FromIpHash, char * sIpTo, const uint8_t * ui128ToIpHash, char * sReason, char * sBy, const bool &bFull) {
    ...
    if(sReason != NULL) {
        size_t szReasonLen = strlen(sReason);
#ifdef _WIN32
        pRangeBan->sReason = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szReasonLen > 255 ? 256 : szReasonLen+1);
#else
...
#ifdef _WIN32
        pRangeBan->sBy = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szByLen+1);
#else
...
bool clsBanManager::RangeTempBan(char * sIpFrom, const uint8_t * ui128FromIpHash, char * sIpTo, const uint8_t * ui128ToIpHash, char * sReason, char * sBy, const uint32_t &minutes,
    ...
    if(expiretime > 0) {
        ...
        if(minutes > 0) {
    	    ...
    	    pRangeBan->tempbanexpire = acc_time+(clsSettingManager::mPtr->iShorts[SETSHORT_DEFAULT_TEMP_BAN_TIME]*60);
        }
    }
