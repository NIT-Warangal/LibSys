...
void clsServiceLoop::ReceiveLoop() {
    ...
    if(clsServerManager::ui64ActualTick-iLstUptmTck > 60) {
        ...
        if(clsServerManager::ui64Mins == 0 || clsServerManager::ui64Mins == 15 || clsServerManager::ui64Mins == 30 || clsServerManager::ui64Mins == 45) {
            ...
            if(HeapValidate(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, 0) == 0) {
                AppendDebugLog("%s - [ERR] Lib memory heap corrupted\n", 0);
            }
            HeapCompact(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE);
            ...
            case User::STATE_ADDME_1LOOP: {
                ...
                if(iBeforeLuaLen < curUser->sbdatalen) {
					...
					if(curUser->uLogInOut->pBuffer == NULL) {
						curUser->uLogInOut->pBuffer = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szNeededLen+1);
					} else {
						curUser->uLogInOut->pBuffer = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, sOldBuf, szNeededLen+1);
					}
#else
...
#ifdef _WIN32
                        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)curUser->sLastChat) == 0) {
...
#ifdef _WIN32
						if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)curUser->sLastPM) == 0) {
...
#ifdef _WIN32
                        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)curUser->sLastSearch) == 0) {
