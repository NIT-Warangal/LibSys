// General DC++ Library extracted from linuxdcpp
// To install linuxdcpp client on your machine, go for
...
void clsDcCommands::BotINFO(User * curUser, char * sData, const uint32_t &iLen) {
	...
	int imsgLen = sprintf(msg, "$HubINFO %s$%s:%hu$%s.px.$%hd$%" PRIu64 "$%hd$%hd$Lib$%s|", clsSettingManager::mPtr->sTexts[SETTXT_HUB_NAME], clsSettingManager::mPtr->sTexts[SETTXT_HUB_ADDRESS],
...
#ifdef _WIN32
                    if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)cur->ToNick) == 0) {
            ...
            case 'B': {
                if(((curUser->ui32BoolBits & User::BIT_PINGER) == User::BIT_PINGER) == false && szDataLen == 7 && memcmp(sSupport+1, "otINFO", 6) == 0) {
                    if(clsSettingManager::mPtr->bBools[SETBOOL_DONT_ALLOW_PINGERS] == true) {
                        ...
                        int imsgLen = sprintf(msg, "<%s> Sorry, this hub banned you from hublist because of your allowance of CTM exploit.|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC]);
    ...
    while(next != NULL) {
        ...
        switch(cur->cType) {
            case PrcsdUsrCmd::SUPPORTS: {
                ...
                // PPK ... yes yes yes finally QuickList support in Lib !!! ;))
...
void clsDcCommands::SendIncorrectIPMsg(User * curUser, char * sBadIP, const bool &bCTM) {
	...
	int iret = sprintf(msg+imsgLen, " %s %s !|", bCTM == true ? clsLanguageManager::mPtr->sTexts[LAN_IN_CTM_REQ_REAL_IP_IS] : clsLanguageManager::mPtr->sTexts[LAN_IN_SEARCH_REQ_REAL_IP_IS], curUser->sIP);
	imsgLen += iret;
...
#ifdef _WIN32
        cmdSearch->sCommand = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pOldBuf, cmdSearch->iLen+szLen+1);
#else
...
#ifdef _WIN32
        cmdSearch->sCommand = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szLen+1);
#else
