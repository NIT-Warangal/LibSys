...
void clsUpdateCheckThread::Run() {
    ...
    if(::getaddrinfo("www.Lib.org", "80", &hints, &pResult) != 0 || (pResult->ai_family != AF_INET && pResult->ai_family != AF_INET6)) {
	...
	Message("Connecting to Lib.org ...", 28);
	...
	Message("Connected to Lib.org, sending request...", 43);
	...
	Message("Request to Lib.org sent, receiving data...", 45);
...
bool clsUpdateCheckThread::SendHeader() {
	char * sDataToSend = "GET /version HTTP/1.1\r\nUser-Agent: Lib " LibVersionString " [" BUILD_NUMBER "]"
