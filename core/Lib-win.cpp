...
static int InstallService(const char * sServiceName, const char * sPath) {
	...
	if(schService == NULL) {
	    ...
	    printf("Lib service '%s' installed successfully.", sServiceName);
	}
...
static int UninstallService(const char * sServiceName) {
	...
	if(DeleteService(schService) == false) {
	    ...
	    printf("Lib service '%s' deleted successfully.", sServiceName);
	    CloseServiceHandle(schService);
	    CloseServiceHandle(schSCManager);
	    return EXIT_SUCCESS;
	}
...
//    Cout("Lib Debug console\n");
	...
	for(int i = 0; i < argc; i++) {
	    if(stricmp(argv[i], "-s") == NULL || stricmp(argv[i], "/service") == NULL) {
	    	...
	    	printf("Lib [-c <configdir>] [-i <servicename>] [-u <servicename>] [-v]");
	    	return EXIT_SUCCESS;
