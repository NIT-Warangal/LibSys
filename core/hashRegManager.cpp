...
RegUser::~RegUser() {
    ...
    if(bPassHash == true) {
#ifdef _WIN32
        if(ui8PassHash != NULL && HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)ui8PassHash) == 0) {
...
#ifdef _WIN32
        if(sPass != NULL && HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sPass) == 0) {
...
#ifdef _WIN32
    pReg->sNick = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szRegNickLen+1);
#else
...
#ifdef _WIN32
        pReg->ui8PassHash = (uint8_t *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, 64);
#else
...
#ifdef _WIN32
        pReg->sPass = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szRegPassLen+1);
#else
...
bool RegUser::UpdatePassword(char * sNewPass, size_t &szNewLen) {
    if(clsSettingManager::mPtr->bBools[SETBOOL_HASH_PASSWORDS] == false) {
        if(bPassHash == true) {
...
#ifdef _WIN32
            sPass = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sOldBuf, szNewLen+1);
#else
            ...
            char * sOldPass = sPass;
#ifdef _WIN32
            sPass = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sOldPass, szNewLen+1);
#else
        ...
        if(bPassHash == true) {
            ...
            char * sOldPass = sPass;
#ifdef _WIN32
            ui8PassHash = (uint8_t *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sOldPass, 64);
#else
...
void clsRegManager::Load(void) {
    ...
    if(pxbRegs.ui16ItemLengths[0] != 23 || strncmp((char *)pxbRegs.pItemDatas[0], "Lib Registered Users", 23) != 0) {
        return;
    }
...
void clsRegManager::Save(const bool &bSaveOnChange/* = false*/, const bool &bSaveOnTime/* = false*/) {
    ...
    pxbRegs.pItemDatas[0] = (void *)"Lib Registered Users";
...
#ifdef _WIN32
            pCurReg->ui8PassHash = (uint8_t *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, 64);
#else
...
#ifdef _WIN32
                if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sOldPass) == 0) {
...
#ifdef _WIN32
                if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pCurReg->ui8PassHash) == 0) {
