...
FARPROC WINAPI Lib_FailHook(unsigned /*dliNotify*/, PDelayLoadInfo /*pdli*/) {
#ifdef _BUILD_GUI
    ::MessageBox(NULL, "Something bad happen and Lib crashed. Lib was not able to collect any information why this happen because your operating system"
		" don't support functionality needed for that. If you know why this crash happen then please report it as bug to PPK@Lib.org!",
        "Lib crashed!", MB_OK | MB_ICONERROR);
#else
    AppendLog("Something bad happen and Lib crashed. Lib was not able to collect any information why this happen because your operating system"
        " don't support functionality needed for that. If you know why this crash happen then please report it as bug to PPK@Lib.org!");
#endif
...
LONG WINAPI Lib_UnhandledExceptionFilter(LPEXCEPTION_POINTERS ExceptionInfo) {
    ...
    // Set failure hook
	__pfnDliFailureHook2 = Lib_FailHook;
    ...
    if(FileExist(sDebugSymbolsFile.c_str()) == false) {
        ...
        ::MessageBox(NULL, "Something bad happen and Lib crashed. Lib was not able to collect any information why this happen because file with debug symbols"
			" (Lib.pdb) is missing. If you know why this crash happen then please report it as bug to PPK@Lib.org!",
            "Lib crashed!", MB_OK | MB_ICONERROR);
#else
    ...
    if(SymInitialize(GetCurrentProcess(), clsServerManager::sPath.c_str(), TRUE) == FALSE) {
        ...
        ::MessageBox(NULL, "Something bad happen and Lib crashed. Lib was not able to collect any information why this happen because initializatin of"
			" debug symbols failed. If you know why this crash happen then please report it as bug to PPK@Lib.org!",
            "Lib crashed!", MB_OK | MB_ICONERROR);
#else
    ...
    if(fw == NULL) {
        ...
        ::MessageBox(NULL, "Something bad happen and Lib crashed. Lib was not able to create file with information why this crash happen."
			" If you know why this crash happen then please report it as bug to PPK@Lib.org!",
            "Lib crashed!", MB_OK | MB_ICONERROR);
#else
    ...
    sCrashMsg += ", please send that file to PPK@Lib.org!";
