...
ProfileItem::~ProfileItem() {
#ifdef _WIN32
    if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sName) == 0) {
...
#ifdef _WIN32
    if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)ProfilesTable) == 0) {
...
#ifdef _WIN32
    ProfilesTable = (ProfileItem **)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pOldTable, iProfileCount*sizeof(ProfileItem *));
#else
...
ProfileItem * clsProfileManager::CreateProfile(const char * name) {
...
#ifdef _WIN32
    if(ProfilesTable == NULL) {
        ProfilesTable = (ProfileItem **)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (iProfileCount+1)*sizeof(ProfileItem *));
    } else {
        ProfilesTable = (ProfileItem **)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pOldTable, (iProfileCount+1)*sizeof(ProfileItem *));
    }
#else
    ...
    if(ProfilesTable == NULL) {
#ifdef _WIN32
        HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pOldTable);
#else
        free(pOldTable);
#endif
...
#ifdef _WIN32
    pNewProfile->sName = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szLen+1);
#else
...
#ifdef _WIN32
    ProfilesTable[iProfile]->sName = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sOldName, szLen+1);
#else
