		...
		do {
			...
			if(f != NULL) {
				if(textfile.size != 0) {
					...
					pNewTxtFile->sText = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, textfile.size+2);
					...
					pNewTxtFile->sCommand = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, strlen(textfile.name)-3);
