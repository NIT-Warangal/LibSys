...
char * Lock2Key(char * sLock) {
    ...
    // $Lock EXTENDEDPROTOCOL33MTL6@h5AIad^P2UoPv?fZU]ivM6 Pk=Lib|
...
#ifdef _WIN32
            char * sMSG = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, sData.size()+64);
#else
...
#ifdef _WIN32
    clsServerManager::sGlobalBuffer = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY, clsServerManager::szGlobalBufferSize);
#else
...
#ifdef _WIN32
    if(clsServerManager::sGlobalBuffer != NULL) {
        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)clsServerManager::sGlobalBuffer) == 0) {
...
#ifdef _WIN32
    clsServerManager::sGlobalBuffer = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sOldBuf, clsServerManager::szGlobalBufferSize);
#else
