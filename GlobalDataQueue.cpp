...
clsGlobalDataQueue::clsGlobalDataQueue() {
    ...
    // OpList buffer
#ifdef _WIN32
    OpListQueue.sBuffer = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY, 256);
#else
    ...
    // UserIP buffer
#ifdef _WIN32
    UserIPQueue.sBuffer = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY, 256);
#else
...
#ifdef _WIN32
        GlobalQueues[ui8i].sBuffer = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY, 256);
        GlobalQueues[ui8i].sZbuffer = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY, 256);
#else
...
clsGlobalDataQueue::~clsGlobalDataQueue() {
#ifdef _WIN32
    if(OpListQueue.sBuffer != NULL) {
        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)OpListQueue.sBuffer) == 0) {
...
#ifdef _WIN32
    if(UserIPQueue.sBuffer != NULL) {
        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)UserIPQueue.sBuffer) == 0) {
...
#ifdef _WIN32
            if(pCur->sData != NULL) {
                if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pCur->sData) == 0) {
    ...
    if(pNewQueueItems[0] != NULL) {
        ...
        while(pNext != NULL) {
            ...
            if(pCur->sCommand1 != NULL) {
                if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pCur->sCommand1) == 0) {
					AppendDebugLog("%s - [MEM] Cannot deallocate pCur->sCommand1 in clsGlobalDataQueue::~clsGlobalDataQueue\n", 0);
                }
            }
            if(pCur->sCommand2 != NULL) {
                if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pCur->sCommand2) == 0) {
    ...
    for(uint8_t ui8i = 0; ui8i < 144; ui8i++) {
        ...
        if(GlobalQueues[ui8i].sBuffer != NULL) {
            if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)GlobalQueues[ui8i].sBuffer) == 0) {
        ...
        if(GlobalQueues[ui8i].sZbuffer != NULL) {
            if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)GlobalQueues[ui8i].sZbuffer) == 0) {
...
void clsGlobalDataQueue::AddQueueItem(char * sCommand1, const size_t &szLen1, char * sCommand2, const size_t &szLen2, const uint8_t &ui8CmdType) {
...
#ifdef _WIN32
    pNewItem->sCommand1 = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szLen1+1);
#else
    ...
    if(sCommand2 != NULL) {
#ifdef _WIN32
        pNewItem->sCommand2 = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szLen2+1);
#else
        pNewItem->sCommand2 = (char *)malloc(szLen2+1);
#endif
        if(pNewItem->sCommand2 == NULL) {
#ifdef _WIN32
            if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pNewItem->sCommand1) == 0) {
