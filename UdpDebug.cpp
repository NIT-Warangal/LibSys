...
#ifdef _WIN32
    if(Nick != NULL) {
        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)Nick) == 0) {
...
#ifdef _WIN32
    char * sMsg = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szLen);
#else
...
bool clsUdpDebug::New(User * u, const int32_t &port) {
    ...
    // initialize dbg item
#ifdef _WIN32
    pNewDbg->Nick = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, u->ui8NickLen+1);
#else
...
#ifdef _WIN32
    pNewDbg->Nick = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, szNameLen+1);
#else
