...
#ifdef _WIN32
	sZbuffer = (char *)HeapAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY, ZBUFFERLEN);
#else
...
#ifdef _WIN32
    if(sZbuffer != NULL) {
        if(HeapFree(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)sZbuffer) == 0) {
...
#ifdef _WIN32
        sZbuffer = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pOldBuf, szZbufferSize);
#else
...
#ifdef _WIN32
        sOutData = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pOldBuf, szOutDataSize+1);
#else
...
char * clsZlibUtility::CreateZPipe(char *sInData, const unsigned int &uiInDataSize, char *sOutData, unsigned int &uiOutDataLen, unsigned int &uiOutDataSize, size_t (* pAllignFunc)(size_t n)) {
    ...
    if(uiOutDataSize < uiOutDataLen) {
        ...
        char * pOldBuf = sOutData;
#ifdef _WIN32
        sOutData = (char *)HeapReAlloc(clsServerManager::hLibHeap, HEAP_NO_SERIALIZE, (void *)pOldBuf, uiOutDataSize);
#else
